...
function encaisser_article($settings, $objets){
	...
	{
		$infos = $bdd->query("SELECT id, cotisation, solde, mail, prenom FROM membres WHERE numero=".$settings['carte']);
		foreach($infos as $i){
			$id = $i['id'];
		...
		$infos_art = $bdd->query("SELECT tarif, tarif_non_cotisant, nom FROM articles WHERE id=".addslashes($settings['id_article']));
		foreach($infos_art as $i){
			$tarif = $i['tarif'];
			if($cotisation!=annee_scolaire())$tarif = $i['tarif_non_cotisant'];
		
			$solde = $ancien_solde - $tarif;
